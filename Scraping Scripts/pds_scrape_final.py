...
columns_more_4 = 6 #6 column entitlement ()
...
chrome_path = "C:/Users/shami/Desktop/NREGA Data/FTO transaction details/chromedriver/chromedriver.exe" #CHANGE YOUR chromedriver PATH HERE
driver = webdriver.Chrome(chrome_path)
url = ('https://mahaepos.gov.in/SRC_Trans_Int.jsp')
driver.get(url)
x = ["2720"] #Code for Maharashtra State
p = ["%03d"% x for x in range(85,90)] #Change AFDSO Region here
q = ["%05d" % y for y in range(100000)] #Random number Generator with 5 digit sequence
...
srclist = list(map(''.join, product(x, r))) #r; Product of x*p*q
for i in srclist:
...
df.to_csv('final_scrape_transaction.csv', index=True, header = False)#header1 for transactions data
df2.to_csv('final_scrape_seeding_10.csv', index=True, header = False)#header2 for seeding data
df3.to_csv('final_scrape_entitlement_7_10.csv', index=True, header = False)#header3 for 7 column entitlement
df4.to_csv('final_scrape_entitlement_6_10.csv', index=True, header = False)#header4 for 6 column entitlement
#fin.
